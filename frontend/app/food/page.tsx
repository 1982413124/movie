...
export default function FoodPage() {
  ...
  const handleProceed = () => {
    ...
    const existingDraftRaw = window.sessionStorage.getItem("movieReservationDraft");
    const existingDraft = existingDraftRaw ? JSON.parse(existingDraftRaw) : {};
    const nextDraft = {
      ...existingDraft,
      foodItems: selectedItems,
      foodTotalPrice: totalPrice,
      totalPrice: (existingDraft.totalPrice ?? 0) + totalPrice,
    };

    window.sessionStorage.setItem("movieReservationDraft", JSON.stringify(nextDraft));
    router.push("/confirm");
  };
