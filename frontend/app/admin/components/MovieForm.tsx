...
const empty: MovieInput = { title: "", genre: "", duration_minutes: 0, age_rating: "G", synopsis: "", poster_image: "", release_date: "", screening_start: "", screening_end: "", trailer_url: "" };
...
export default function MovieForm({ session, movieId }: { session: AdminSession; movieId?: string }) {
  ...
  return <div className="admin-page-enter">
    ...
    <div className="admin-page-heading form-page-heading"><div><p className="eyebrow">{movieId ? "EDIT FILM" : "NEW FILM"}</p><h1>{movieId ? "作品を編集" : "新しい作品を追加"}</h1><p className="admin-muted">作品の魅力が伝わる情報を、ひとつずつ。</p></div><span className="required-note">予告編・年齢区分以外は必須</span></div>
